...
export const modulo = (dividend: number, divisor: number) => ((dividend % divisor) + divisor) % divisor; // This should be used in favor of the % operator when dealing with negative numbers, since the % operator does not handle negative numbers correctly.

export const round = (value: number, precision: number = 0) =>
  Math.round(value * Math.pow(10, precision)) / Math.pow(10, precision);

export const floor = (value: number, precision: number = 0) =>
...
export const hexPairToDecimal = (value: string) => parseInt(value, 16);

export const shortHexToDecimal = (value: string) => hexPairToDecimal(value + value);

export const hexWithoutHashToRgb = (value: string) => {
...
export const hexWithHashToRgb = (value: string) => hexWithoutHashToRgb(value.substring(1));

export const hexWithoutHashToHsl = (value: string) => rgbToHsl(hexWithoutHashToRgb(value));
export const hexWithHashToHsl = (value: string) => rgbToHsl(hexWithHashToRgb(value));

export const hexWithoutHashToRgba = (value: string) => {
  const { red, green, blue } = hexWithoutHashToRgb(value);
  const alpha = hexPairToDecimal(value.substring(6, 8)) / 255;
  return { red, green, blue, alpha };
};
export const hexWithHashToRgba = (value: string) => hexWithoutHashToRgba(value.substring(1));

export const hexWithoutHashToHsla = (value: string) => rgbaToHsla(hexWithoutHashToRgba(value));
export const hexWithHashToHsla = (value: string) => rgbaToHsla(hexWithHashToRgba(value));

export const shortHexWithoutHashToRgb = (value: string) => {
...
export const shortHexWithHashToRgb = (value: string) => shortHexWithoutHashToRgb(value.substring(1));

export const shortHexWithoutHashToHsl = (value: string) => rgbToHsl(shortHexWithoutHashToRgb(value));
export const shortHexWithHashToHsl = (value: string) => rgbToHsl(shortHexWithHashToRgb(value));

export const shortHexWithoutHashToRgba = (value: string) => {
  const { red, green, blue } = shortHexWithoutHashToRgb(value);
  return { red, green, blue, alpha: 1 };
};
export const shortHexWithHashToRgba = (value: string) => shortHexWithoutHashToRgba(value.substring(1));

export const shortHexWithoutHashToHsla = (value: string) => rgbaToHsla(shortHexWithoutHashToRgba(value));
export const shortHexWithHashToHsla = (value: string) => rgbaToHsla(shortHexWithHashToRgba(value));

export const numberFromPercentageOrUnitInterval = (value: string) =>
  isValidUnitIntervalValue(value) ? parseFloat(value) : numberFromPercentage(value);

export const numberFromPercentage = (value: string) => {
